...
export class ExecucaoService {
    ...
    private async _validateMonthlyTokenConsumption(agent: Agente) {
        ...
        const monthToDateConsumption = await this.execucaoRepository
            ...
            .andWhere('execution.createdAt >= :firstDayOfMonth AND execution.createdAt <= :lastDayOfMonth', { firstDayOfMonth, lastDayOfMonth })
        ...
        if (sumTokens > agent.monthlyTokenLimit) {
            throw new BadRequestException('O consumo acumulado no mês não pode ultrapassar o limite mensal definido para o agente')
        }
    }
